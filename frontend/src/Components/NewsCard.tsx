...
export default function NewsCard({ article, featured = false, saved, onSave }: { article: Article; featured?: boolean; saved: boolean; onSave: () => void }) { return <article className={`news-card ${featured ? 'news-card--featured' : ''}`}>
  <div className="news-card__image"><img src={getArticleImage(article)} alt={`Vintage-Illustration: ${article.category}`} /><span>{article.category}</span><button onClick={onSave} aria-label={saved ? 'Aus den Lesezeichen entfernen' : 'Nachricht speichern'}>{saved ? <HiBookmark /> : <HiOutlineBookmark />}</button></div>
