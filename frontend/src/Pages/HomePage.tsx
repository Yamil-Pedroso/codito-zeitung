...
export default function HomePage() {
  const [filter, setFilter] = useState<(typeof filters)[number]>(() => categoryFromHash(window.location.hash) ?? 'Alle'); const [query, setQuery] = useState(''); const [saved, setSaved] = useState<number[]>([])
  useEffect(() => {
    const syncNavigation = () => {
      ...
      setQuery('')
      window.setTimeout(() => {
        if (category) document.getElementById('schweiz')?.scrollIntoView({ behavior: 'smooth', block: 'start' })
